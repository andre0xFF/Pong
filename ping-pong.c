...
void start_game(struct tagObject racket[], struct tagObject *ball,
                struct tagPlayer player[], int tab[][COL], int game_cycle);
int arrow_code(void);
void move_raket(int tab[][COL], struct tagObject racket[], int i,
                int move); // Mover a raquere
void move_raket_left(int tab[][COL], struct tagObject racket[], int i,
                     int move);
void move_raket_right(int tab[][COL], struct tagObject racket[], int i,
                      int move);

int main(void) {
  int tab[LIN][COL];  // tabuleiro
  int seed;           // Variavel do Algoritmo SERIE
  int ch = -1;        // Codigo das teclas
  int game_cycle = 0; // Ciclo N do tabuleiro
  int c;              // som
  int game_over;      // Fim do jogo, voltar ao menu
  ...
  // Caracteres do tabuleiro (219, 177, 240) sao do codepage 437 (DOS)
  ...
  do {
    ...
    case '1':
      ...
      do {
        ...
        while (
            _kbhit() !=
            0) // KEY_ESC sai do programa a qualquer altura quando pressionada
        {
          ch = arrow_code();
          // printf( "\nKey struck was '%d + %d'\n", getch(), getch() );;
          switch (ch) {
          ...
          case 'p': // pause
            while (_getch() != 'p')
              Sleep(1);
            break;
          case KEY_ESC: // sair do jogo, voltar ao menu
            game_over = 1;
            break;
          }
        }
        ...
        // Quando a bola passa na rede repor o desenho da rede, bola passa por
        // de cima da rede
        ...
        if (tab[ball.pos_y + ball.move_y][ball.pos_x + ball.move_x] == 219)
          ball.move_x *= -1;
        /*
         *	//Refleccao das raquetes [ESPECIAL DE CORRIDA] -> NAO ESTA A
         * FUNCIONAR CORRECTAMENTE if( tab[ball.pos_y + ball.move_y][ball.pos_x
         * + ball.move_x] == 219 )
         *	{
        ...
        if (ball.pos_x == 2 || ball.pos_x == COL - 2) {
          if (ball.pos_x ==
              COL - 2) // Bola na baliza do player2 -> ponto player1
            player[0].points++;
          else // Bola na baliza do player1 -> ponto player2
            player[1].points++;

          // Representar vencedor e voltar ao menu
          if (player[0].points > MAX_SCORE) {
            Position.X = 5;
            Position.Y = 9;
...
void print_tab(int tab[][COL]) {
  int lin_y;
  int col_x;
  char line[COL]; // Buffer de uma linha, imprimir de uma vez evita o flicker
  ...
  for (lin_y = 1; lin_y < LIN; lin_y++) {
    for (col_x = 1; col_x < COL; col_x++)
      line[col_x - 1] = (char)tab[lin_y][col_x];
    line[COL - 1] = '\0';

    if (lin_y != LIN - 1) // Nao dar ENTER na ultima linha
      printf("%s\n", line);
    else
      printf("%s", line);
  }
